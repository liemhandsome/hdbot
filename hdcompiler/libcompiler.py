...
def process(line):
    ...
    global result, labels, adr_of_cmds, adr_arith_cmds, pr_length_cmds, home, string_vars, in_comment, def_aliases
    ...
    # Lấy phần sau chữ "hex"
        hexdata = line[3:].strip()

        if not hexdata:
            raise ValueError('Missing hex data')
    ...
    # Emit từng byte
        ...
        if ':' not in content:
            raise ValueError("def: cú pháp là def <tên>:<hex>")
        name, hexstr = content.split(':', 1)
        def_aliases[name.strip()] = hexstr.strip()
    elif line.startswith('str_calc'):
        ''' Syntax:
        - str_calc <var> "<expr>" :  lưu byte opcode của biểu thức calc vào biến <var>
        - str_calc "<expr>"       :  xuất thẳng byte opcode không qua biến
        - str_calc <var>          :  dùng lại biến đã khai báo trước
        '''
        CASIO_OPCODES = {
        ...
        def calc_to_bytes(expr):
            sorted_keys = sorted(CASIO_OPCODES.keys(), key=len, reverse=True)
            ...
            while i < len(expr):
                ...
                for key in sorted_keys:
                    if expr.startswith(key, i):
                        byte_list.append(CASIO_OPCODES[key])
                        i += len(key)
                        matched = True
                        break
                if not matched:
                    raise ValueError(f"str_calc: ký tự không nhận ra '{expr[i]}' tại vị trí {i}")
            return byte_list

        content = line[len('str_calc'):].strip()

        if '"' in content:
            quote_pos = content.find('"')
            var_name = content[:quote_pos].strip() if quote_pos > 0 else None
            expr = content[quote_pos+1:].rstrip('"')

            bytes_list = calc_to_bytes(expr)
            print(f"str_calc: '{expr}' -> {[hex(b) for b in bytes_list]}")

            if var_name:
                string_vars[var_name] = expr          # lưu raw expr
            ...
            if var_name in string_vars:
                bytes_list = calc_to_bytes(string_vars[var_name])
                result.extend(bytes_list)
            else:
                raise ValueError(f"str_calc: biến chưa khai báo: {var_name}")
        else:
            raise ValueError("str_calc: cú pháp không hợp lệ")
    ...
    elif line in def_aliases:
        raw = def_aliases[line]
        val = int(raw, 16) + 0x30300000
        process(f'0x{val:08x}')
    else:
